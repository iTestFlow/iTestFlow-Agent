...
- Test Execution drives a real browser through a pinned `@playwright/mcp` child process per run (isolated profile, `--caps=testing`, explicit tool allowlist — no eval/upload/storage tools). Steps are natural language; the agent chooses one validated action per turn across the UI, API, and database layers. Secrets travel as `{{secret:NAME}}` placeholders and are resolved only in worker memory with output scrubbing.
- Test Execution authorization is derived, not tester-configured. Approving a run freezes the environment, steps, and notes; from that frozen config the worker derives an **execution boundary** (API origin, OAuth token URL, OpenAPI URL, database host/port) that authorizes every outbound hop, and asks the supplied database account which non-system objects it can see (recorded once per run — a reclaimed worker reuses it). Private/loopback destinations are denied unless their range is listed in `TEST_EXECUTION_PRIVATE_NETWORK_CIDRS`; link-local/metadata, multicast, and reserved addresses are never reachable. Every run freezes an `executionPolicyVersion`; a run frozen before that stamp existed keeps the mutation gates, schema allowlist, and read-only ad-hoc API surface it was originally approved with.
