...
The `/test-execution` page is a four-step workbench (Setup → Test Cases → Review & Execute → Results). Runs are immutable server-side; all authoring happens in a client-side draft (per-project localStorage, secret values never persisted). The run-creation route accepts the caller's final case list — imported from a Test Plan/Suite (via the read-only `case-preview` route), imported by User Story (linked test cases), or hand-written with no Azure identity (the case columns are nullable) — plus run-level settings: a required Base URL (validated against `PLAYWRIGHT_EXECUTION_ALLOWED_ORIGINS` when the web process knows it; the worker always enforces it), optional execution notes (capped like extra instructions), a screenshot policy (`every-step` | `validation-points` default | `failures-only` | `none`), and title/value test data. Secret test-data values are AES-GCM encrypted in `playwright_execution_run_data`; reruns and profile prefills reference the saved ciphertext by source id + title, so secret material never returns to the browser. Reusable per-project setup profiles live in `playwright_execution_profiles`/`_profile_data` with the same encrypted-quad shape. Run settings live in dedicated run columns — never in `config_snapshot_json`, whose worker drift check compares transport fields only.

The worker executes cases serially with one isolated MCP connection per case, deterministically navigating to the run's Base URL before each case's first step. LLM decisions can invoke only advertised tools in the fixed allowlist; `browser_run_code_unsafe` and unrelated MCP tools are rejected. The screenshot policy is enforced deterministically in the job loop after each step (validation point = step with an expected result; failure evidence is captured for `validation-points` and `failures-only`; `none` also suppresses persisting agent-initiated screenshots). Decrypted test data and execution notes are threaded into the agent prompt (steps reference data by plain title) and their secret values are scrubbed from persisted tool transcripts and error messages. iTestFlow is the system of record during execution and review. Azure Test Points are updated only by the explicit publish route — restricted to cases that carry a test point (per-case plan/suite ids) — with local outcomes mapped to Azure outcomes.
