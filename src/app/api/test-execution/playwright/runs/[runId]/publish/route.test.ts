...
const recordPublicationResult = vi.fn();
...
beforeEach(() => { vi.resetAllMocks(); requireWorkflowContext.mockResolvedValue({ userId: "u", workspace: { id: "w" } }); resolveProjectScope.mockResolvedValue(scope); getExecutionRun.mockResolvedValue({ status: "passed", azurePlanId: 1 }); publishableCases.mockResolvedValue([{ azureTestCaseId: 4, azureTestPointId: 5, azureSuiteId: 2, outcome: "passed" }]); beginPublication.mockResolvedValue({ id: "pub-1", leaseToken: "lease-1" }); recordPublicationResult.mockResolvedValue(true); updateTestPoints.mockResolvedValue({ success: true }); getUserAzureAdapter.mockResolvedValue({ updateTestPoints }); });
describe("reviewed Azure publication", () => {
  ...
  it("reserves one publication before updating points", async () => { const response = await POST(request({ scope, confirmedReviewed: true }), { params: Promise.resolve({ runId: "r" }) }); expect(response.status).toBe(200); expect(beginPublication).toHaveBeenCalledBefore(updateTestPoints); expect(finishPublication).toHaveBeenCalledWith({ id: "pub-1", leaseToken: "lease-1", status: "completed", result: expect.any(Array) }); });
  ...
  it("retries only failed points while preserving prior successes", async () => {
    ...
    beginRetry.mockResolvedValue({ id: "pub-1", leaseToken: "lease-2", prior: [{ testCaseId: 4, testPointId: 5, success: true }, { testCaseId: 4, testPointId: 7, success: false, error: "prior failure" }] });
    ...
    expect(finishPublication).toHaveBeenCalledWith({ id: "pub-1", leaseToken: "lease-2", status: "completed", result: [{ testCaseId: 4, testPointId: 5, success: true }, { testCaseId: 4, testPointId: 7, success: true, error: undefined }] });
  });
  it("retries unattempted points after an interrupted publication", async () => {
    publishableCases.mockResolvedValue([{ azureTestCaseId: 4, azureTestPointId: 5, azureSuiteId: 2, outcome: "passed" }, { azureTestCaseId: 4, azureTestPointId: 7, azureSuiteId: 2, outcome: "failed" }]);
    beginRetry.mockResolvedValue({ id: "pub-1", leaseToken: "lease-2", prior: [{ testCaseId: 4, testPointId: 5, success: true }] });
    const response = await POST(request({ scope, confirmedReviewed: true, retryFailed: true }), { params: Promise.resolve({ runId: "r" }) });
  ...
  it("persists point receipts as an array when Azure publication throws", async () => {
    ...
    expect(finishPublication).toHaveBeenCalledWith({ id: "pub-1", leaseToken: "lease-1", status: "failed", result: [] });
  });
  it("persists each point receipt before attempting the next Azure write", async () => {
    publishableCases.mockResolvedValue([
      { azureTestCaseId: 4, azureTestPointId: 5, azureSuiteId: 2, outcome: "passed" },
      { azureTestCaseId: 6, azureTestPointId: 7, azureSuiteId: 2, outcome: "failed" },
    ]);
    await POST(request({ scope, confirmedReviewed: true }), { params: Promise.resolve({ runId: "r" }) });
    expect(recordPublicationResult).toHaveBeenCalledTimes(2);
    expect(recordPublicationResult).toHaveBeenNthCalledWith(1, "pub-1", "lease-1", { testCaseId: 4, testPointId: 5, success: true, error: undefined });
    expect(recordPublicationResult).toHaveBeenNthCalledWith(2, "pub-1", "lease-1", { testCaseId: 6, testPointId: 7, success: true, error: undefined });
    expect(recordPublicationResult.mock.invocationCallOrder[0]).toBeLessThan(updateTestPoints.mock.invocationCallOrder[1]);
  });
});
