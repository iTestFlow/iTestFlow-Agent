...
import { updateExecutionProfile } from "@/modules/test-execution/execution-profiles.service";
import { MAX_PROFILE_NAME_LENGTH, ProfileNameConflictError, ProfileNotFoundError } from "@/modules/test-execution/execution-profiles.shared";
import {
...
// The bare secret variant (last) means "keep this profile's saved value".
const TestDataEntrySchema = z.union([
  ...
  z.object({ title: TitleSchema, isSecret: z.literal(true), sourceTitle: TitleSchema.optional() }),
]);
...
export async function POST(request: Request, context: { params: Promise<{ profileId: string }> }) {
  ...
  try {
    ...
    const profile = await updateExecutionProfile(profileId, {
