...
import { fakeAzureAdapter, jsonRequest, projectScope, requirement, testCase } from "@/test/factories";
 ...
 * run route's own wiring: a silent divergence here means the copied prompt differs from
 * what production actually sends for the same work item.
 */
...
describe("existing-test-case-review manual draft route", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    ...
    mocks.getUserAzureAdapter.mockResolvedValue(fakeAzureAdapter({
      ...
      fetchLinkedTestCases: vi.fn(async () => []),
    }));
  ...
  it("sends the workspace model window, the retrieval floor and the ranked knowledge it selected", async () => {
    ...
    const response = await POST(jsonRequest("/api/existing-test-case-review/manual/draft", body()));

    expect(response.status).toBe(200);
    expect(mocks.buildExistingTestCaseReviewPromptDraft).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("anchors knowledge ranking on the work items context actually resolved", async () => {
    ...
    await POST(jsonRequest("/api/existing-test-case-review/manual/draft", body()));
  ...
  it("passes the linked test cases it fetched through to the draft and the response", async () => {
    const linkedTestCases = [testCase({ id: "301", title: "Existing case" })];
    mocks.getUserAzureAdapter.mockResolvedValue(fakeAzureAdapter({
      fetchWorkItemById: vi.fn(async () => requirement({ id: "123", title: "Target" })),
      fetchLinkedTestCases: vi.fn(async () => linkedTestCases),
    }));

    const response = await POST(jsonRequest("/api/existing-test-case-review/manual/draft", body()));
    const payload = await response.json();

    expect(payload.linkedTestCases).toEqual(linkedTestCases);
    expect(mocks.buildExistingTestCaseReviewPromptDraft).toHaveBeenCalledWith(expect.objectContaining({
      linkedTestCases,
    }));
  });
  ...
  it("rejects a request with no target work item before doing any work", async () => {
    const response = await POST(jsonRequest("/api/existing-test-case-review/manual/draft", {
