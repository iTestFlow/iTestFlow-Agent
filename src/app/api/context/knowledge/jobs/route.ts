...
export async function POST(request: Request) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "A valid project knowledge operation and scope are required." }, { status: 400 });
  }
  try {
    ...
    return routeErrorResponse(error, { fallback: "The project knowledge job could not be queued." });
  }
}
