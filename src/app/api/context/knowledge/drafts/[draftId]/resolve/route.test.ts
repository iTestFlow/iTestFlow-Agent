...
const body = { scope: { ...trustedScope, workspaceId: "workspace-1" }, proposedKnowledge: { arbitrarilyLarge: true } };
const params = { params: Promise.resolve({ draftId: "draft-1" }) };

describe("retired full-KB resolve endpoint", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("rejects malformed scope before authentication", async () => {
    const response = await POST(jsonRequest("/resolve", { scope: null }), params);
  ...
  it("remains role guarded", async () => {
    ...
    const response = await POST(jsonRequest("/resolve", body), params);
    expect(response.status).toBe(403);
  });

  it("returns 410 and never accepts a client knowledge-base clone", async () => {
    const response = await POST(jsonRequest("/resolve", body), params);
    expect(response.status).toBe(410);
    expect(await response.json()).toEqual({
      error: "Full knowledge-base review submissions were removed. Submit compact conflict decisions instead.",
      code: "full_review_removed",
    });
  });
});
