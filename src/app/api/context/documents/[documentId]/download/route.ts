...
/** Convenience download for the current version; immutable links use /versions/:id/download. */
export async function GET(request: Request, { params }: RouteParams) {
  ...
  try {
    ...
    if (!document || !document.currentVersionId) {
      return NextResponse.json({ error: "The requested source document was not found in the selected project." }, { status: 404 });
    }
    const version = await getProjectSourceDocumentVersion({ scope, versionId: document.currentVersionId });
    if (!version || version.documentId !== document.id) {
      return NextResponse.json({ error: "The current document version could not be found." }, { status: 404 });
    }
    ...
    return documentAuthOrErrorResponse(error, "The document could not be downloaded.");
  }
}
