...
/** Native number `<select>` styled to match the form inputs. */
...
/**
 * Secret input with its own show/hide toggle and a "replace token" affordance.
 * When a value is already saved and the field is untouched, it shows a masked,
 * read-only state with a Replace button; editing reveals an empty input. Leaving
 * the field empty signals "keep the saved token" to the parent.
 */
...
/** A status pill colored by tone, used in the status summary and section headers. */
...
/** Inline pass/fail line for a single connection test result. */
export function ConnectionResult({
  ...
  return (
    ...
    >
      ...
      {result.error ? <span className="mt-1 block">{result.error}</span> : null}
    </div>
  );
}
