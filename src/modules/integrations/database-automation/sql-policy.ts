...
// SELECT can invoke user-defined functions with write or network side effects.
// Permit only ordinary built-ins we explicitly know are read-only.
const SAFE_FUNCTIONS = new Set([
  "ABS", "AVG", "CAST", "CEIL", "CEILING", "COALESCE", "CONCAT", "COUNT", "CURRENT_DATE",
  "CURRENT_TIMESTAMP", "DATE", "DATE_TRUNC", "EXISTS", "FLOOR", "IFNULL", "ISNULL", "LENGTH",
  "LOWER", "LTRIM", "MAX", "MIN", "NULLIF", "REPLACE", "ROUND", "RTRIM", "SUBSTRING",
  "SUM", "TRIM", "UPPER",
]);
...
export function validateSql(input: {
  ...
  for (const table of tables) {
    ...
    if (!input.allowedTables.has(`${schema.toLowerCase()}.${(tableName ?? "").toLowerCase()}`)) {
...
function assertSafeFunctions(ast: unknown): void {
  ...
  const visit = (value: unknown): void => {
    ...
    if (node.type === "function" || node.type === "aggr_func") {
      ...
      if (!name || !SAFE_FUNCTIONS.has(name)) throw policy("SQL function is outside the read-only allowlist.");
    }
...
function functionName(value: unknown): string | null {
  ...
  const name = value as { schema?: unknown; name?: unknown };
  if (name.schema || !Array.isArray(name.name) || name.name.length !== 1) return null;
  const part = name.name[0] as { value?: unknown };
  return typeof part?.value === "string" ? part.value.toUpperCase() : null;
}
