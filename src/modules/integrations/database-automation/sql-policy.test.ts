...
import { boundedDatabaseRows, protectDatabaseResult } from "./database-result";
...
describe("database SQL policy", () => {
  it.each([
    ...
    const validated = validateSql({
      ...
      allowedTables: new Set([schema + ".orders"]),
      parameters: { id: 42 },
    });
  ...
  it("rejects DDL, multiple statements, SELECT INTO, and out-of-scope schemas", () => {
    const base = { intent: "select" as const, driver: "postgres" as const, allowedSchemas: ["public"], allowedTables: new Set(["public.orders"]), parameters: {} };
  ...
  it.each([
    ...
    expect(() => validateSql({
      ...
      allowedTables: new Set(["public.orders", "dbo.orders", "testdb.orders"]),
      parameters: {},
  ...
  it("rejects user-defined and side-effectful functions in a SELECT", () => {
    const base = { intent: "select" as const, driver: "postgres" as const, allowedSchemas: ["public"], allowedTables: new Set(["public.orders"]), parameters: {} };
    expect(() => validateSql({ ...base, sql: "SELECT public.run_write()" })).toThrow("function");
    expect(() => validateSql({ ...base, sql: "SELECT pg_notify('channel', 'payload')" })).toThrow("function");
    expect(validateSql({ ...base, sql: "SELECT COUNT(*) FROM public.orders" }).command).toBe("SELECT");
  });
  ...
  it("allows only INSERT/UPDATE/DELETE in approved mutation mode and exact bindings", () => {
    expect(validateSql({
      ...
      allowedTables: new Set(["public.orders"]),
  ...
  it("caps rows and redacts sensitive columns", () => {
    ...
    expect(boundedDatabaseRows([{ value: "prefix-secret-suffix" }], 1, 1000, ["secret"]).safeRows)
      .toEqual([{ value: "prefix-[REDACTED]-suffix" }]);
    const report = protectDatabaseResult({ rows: [{ value: "secret" }], safeRows: [{ value: "[REDACTED]" }] });
    expect(report.rows[0].value).toBe("secret");
    expect(JSON.stringify(report)).not.toContain("secret");
  });

  describe("select row bounding", () => {
    const base = { intent: "select" as const, allowedSchemas: ["app"], allowedTables: new Set(["app.orders", "app.users"]), parameters: {} };
    ...
    it("never bounds mutations", () => {
      const validated = validateSql({
        ...
        driver: "postgres",
        allowedSchemas: ["app"],
        allowedTables: new Set(["app.orders", "app.users"]),
        parameters: { id: 1 },
