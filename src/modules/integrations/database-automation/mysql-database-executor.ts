...
export class MysqlDatabaseExecutor implements DatabaseExecutor {
    ...
    try {
      const [rows] = await connection.query({
        sql: `SELECT table_schema, table_name, column_name, data_type
              ...
              LIMIT ?`,
        values: [this.config.databaseName, MAX_DISCOVERED_COLUMNS + 1],
  ...
  async execute(request: DatabaseExecutionRequest): Promise<DatabaseExecutionResult> {
    assertNotCanceled(this.config.signal, "MySQL");
    if (request.kind === "mutation" && !this.config.allowWrites) {
      throw new DatabaseExecutorError("Database writes are disabled for this connection.", "policy", false, undefined, "writes-disabled");
    }
    const connection = await this.ensureConnected();
    assertNotCanceled(this.config.signal, "MySQL");
...
function normalizeMysql(rows: unknown, fields: FieldPacket[] | undefined, command: string, durationMs: number, config: DatabaseExecutorConfig): DatabaseExecutionResult {
  ...
  const bounded = boundedDatabaseRows(records, config.maxRows ?? 200, config.maxPreviewBytes ?? 64 * 1024, [config.credentials.password]);
  return protectDatabaseResult({ status: "ok", command, rowCount: header?.affectedRows ?? records.length, columns: fields?.map((field) => field.name) ?? Object.keys(records[0] ?? {}), ...bounded, durationMs });
}
