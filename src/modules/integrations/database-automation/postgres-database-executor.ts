...
export class PostgresDatabaseExecutor implements DatabaseExecutor {
  ...
  async execute(request: DatabaseExecutionRequest): Promise<DatabaseExecutionResult> {
    ...
    // pg has no reliable AbortSignal query option. Closing the active socket
    // makes cancellation prompt; rollback failure then marks a write uncertain.
    const cancel = () => this.resetConnection();
...
function normalizeResult(
  ...
  const bounded = boundedDatabaseRows(result.rows ?? [], config.maxRows ?? 200, config.maxPreviewBytes ?? 64 * 1024, [config.credentials.password]);
  return protectDatabaseResult({
    status: "ok",
