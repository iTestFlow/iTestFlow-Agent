...
vi.mock("./story-attachment.handler", () => ({ runStoryAttachmentParseJob: vi.fn(), runStoryAttachmentCleanupJob: vi.fn() }));
vi.mock("./story-attachment-jobs.service", () => ({
  STORY_ATTACHMENT_PARSE: "story_attachment_parse",
  STORY_ATTACHMENT_CLEANUP: "story_attachment_cleanup",
}));
...
describe("registerAllJobHandlers", () => {
  ...
  it("registers Jira, story attachment, and Playwright execution handlers exactly once", async () => {
    ...
    expect(registerJobHandler).toHaveBeenCalledWith("story_attachment_parse", expect.any(Function));
    expect(registerJobHandler).toHaveBeenCalledWith("story_attachment_cleanup", expect.any(Function));
    ...
    expect(registerJobHandler.mock.calls.filter(([type]) => type === "story_attachment_parse")).toHaveLength(1);
    expect(registerJobHandler.mock.calls.filter(([type]) => type === "story_attachment_cleanup")).toHaveLength(1);
    expect(registerJobHandler.mock.calls.filter(([type]) => type === "playwright_mcp_execution")).toHaveLength(1);
    expect(registerJobHandler.mock.calls.some(([type]) => type === "jira_webhook_reconcile")).toBe(false);
  });
});
